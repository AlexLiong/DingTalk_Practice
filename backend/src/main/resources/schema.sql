...
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS theme VARCHAR (16) DEFAULT 'light';
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS last_route VARCHAR (512);
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS view_state_json CLOB;
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS connected_online TINYINT DEFAULT 0;
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS last_online_at DATETIME;
ALTER TABLE sys_user_preference
    ADD COLUMN IF NOT EXISTS last_offline_at DATETIME;
ALTER TABLE sys_user_preference
...
(
    ...
    `chat_status`       tinyint  NOT NULL DEFAULT 1 COMMENT '用户手动设置状态：1在线 2忙碌 3离开',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    INDEX `idx_userid_time` (`user_id`, `create_time` DESC)
) COMMENT ='用户在线状态表';
-- 角色表
...
ALTER TABLE sys_file
    ADD COLUMN IF NOT EXISTS scene VARCHAR (64);
ALTER TABLE sys_file
...
ALTER TABLE sys_file
    ADD COLUMN IF NOT EXISTS history_json CLOB;
ALTER TABLE sys_file
    ADD COLUMN IF NOT EXISTS update_time DATETIME DEFAULT CURRENT_TIMESTAMP;
...
ALTER TABLE sys_mailbox_mail
    ADD COLUMN IF NOT EXISTS sender_id BIGINT;
ALTER TABLE sys_mailbox_mail
    ADD COLUMN IF NOT EXISTS direction VARCHAR (16) DEFAULT 'received';
...
ALTER TABLE sys_ding
    ADD COLUMN IF NOT EXISTS owner_user_id BIGINT;
ALTER TABLE sys_ding
    ADD COLUMN IF NOT EXISTS target_user_id BIGINT;
ALTER TABLE sys_ding
    ADD COLUMN IF NOT EXISTS link_key VARCHAR (64);
