<template>
  ...
  <div class="side-nav desktop-nav">
    ...
    <div class="nav-list">
      <div
        class="nav-row"
        :class="{ active: activeKey === 'chat' }"
        @click="navigateTo('chat')"
      >
        <span class="nav-icon">💬</span><span>消息</span>
        <span v-if="messageUnread > 0" class="nav-badge">{{
          formatCounter(messageUnread)
        }}</span>
      </div>
      <div
        class="nav-row"
        :class="{ active: activeKey === 'documents' }"
      ...
      <div
        class="nav-row"
        :class="{ active: activeKey === 'contacts' }"
      ...
      <div
        class="nav-row"
        :class="{ active: activeKey === 'atme' }"
        @click="navigateTo('atme')"
      >
        <span class="nav-icon">@</span><span>@我</span>
      </div>
      <div
        class="nav-row"
        :class="{ active: activeKey === 'single' }"
      ...
      <div
        class="nav-row"
        :class="{ active: activeKey === 'mailbox' }"
        @click="navigateTo('mailbox')"
      >
        <span class="nav-icon">📧</span><span>邮箱</span>
        <span v-if="collabStore.mailboxUnread > 0" class="nav-badge">{{
          formatCounter(collabStore.mailboxUnread)
        }}</span>
      </div>
      <div
        class="nav-row"
        :class="{ active: activeKey === 'todo' }"
      ...
      <div
        class="nav-row"
        :class="{ active: activeKey === 'ding' }"
        @click="navigateTo('ding')"
      >
        <span class="nav-icon">🔔</span><span>DING</span>
        <span v-if="collabStore.dingPending > 0" class="nav-badge">{{
  ...
  <div class="mobile-bottom-tab">
    ...
    >
      ...
      <span v-if="messageUnread > 0" class="tab-badge">{{
        formatCounter(messageUnread)
      }}</span>
    </div>
    <div
  ...
  <Transition name="panel-slide">
    ...
    >
      <div class="pp-content">
        ...
        <div class="pp-menu-item" @click="toggleThemeFromPanel">
          <el-icon :size="18"><Moon /></el-icon> 个性主题
          <span class="pp-tag">{{ isDark ? "深色" : "浅色" }}</span>
        </div>
        <div v-if="isAdmin" class="pp-menu-item" @click="openAdmin">
...
const messageUnread = computed(() =>
  typeof props.chatUnread === "number"
    ? props.chatUnread
    : collabStore.messageUnread,
);
...
function navigateTo(key) {
  ...
  if (key === "chat") {
    ...
    router.push({ path: "/chat", query: { tab: "contacts" } });
  else if (key === "atme")
    router.push({ path: "/chat", query: { filter: "atme" } });
  else if (key === "calendar") router.push("/calendar");
  else if (key === "single")
    router.push({ path: "/chat", query: { filter: "single" } });
  else if (key === "group")
    router.push({ path: "/chat", query: { filter: "group" } });
