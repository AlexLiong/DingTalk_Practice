...
const displaySessions = computed(() => {
  ...
  else if (chatFilter.value === "group")
    list = list.filter((s) => s.type === 2);
  else if (chatFilter.value === "unread")
    list = list.filter((s) => s.unread > 0);
  else if (chatFilter.value === "top") list = list.filter((s) => s.isTop === 1);
  else if (chatFilter.value === "mute")
    list = list.filter((s) => s.isMute === 1);
  else if (chatFilter.value === "star")
    list = list.filter((s) => s.isStar === 1);
  else if (chatFilter.value === "atme")
...
const isGroupOwner = computed(() => {
  ...
  return me && me.memberRole === 1;
});
const nonMembers = computed(() => {
  const ids = new Set(groupMembers.value.map((m) => m.userId));
  return contacts.value.filter((u) => !ids.has(u.id));
});
...
const addItemMap = computed(() => {
  ...
  [
    ...fileAddItems.value,
    ...savedLinkItems.value,
    ...addAppItems.value,
  ].forEach((item) => map.set(item.key, item));
  return map;
});
...
async function startChat(u) {
  const sid = await apiSingleChat(u.id);
  await loadSessions();
...
async function rememberAddUsage(key) {
  recentUsageKeys.value = [
    key,
    ...recentUsageKeys.value.filter((item) => item !== key),
  ].slice(0, 20);
...
async function saveLink() {
  ...
  savedLinks.value = [
    item,
    ...savedLinks.value.filter((link) => link.id !== item.id),
  ].slice(0, 12);
  linkDraft.value = "";
  linkEditorOpen.value = false;
...
function onWsReceipt(data) {
  ...
  if (current.value.type === 1) {
    messages.value.forEach((m) => {
      if (m.senderId === user.value?.id && m.id <= data.lastMsgId)
        readReceipts.value[m.id] = true;
    });
  } else {
    messages.value.forEach((m) => {
      if (m.senderId === user.value?.id && m.id <= data.lastMsgId)
        readCounts.value[m.id] = (readCounts.value[m.id] || 0) + 1;
    });
  }
}
...
.add-settings-item:disabled .add-settings-item-arrow {
  color: #c0c4cc;
  opacity: 0.72;
}
